...
Math Figures moved from a single-page Vite app to SvelteKit on Vercel, even though every figure could be drawn entirely in the browser. Teachers mostly arrive by searching for a generator ("coordinate grid generator"), so each page needs its own real HTML title and description. A generator page is rendered on the server for each visit, because its settings live in the query string: a shared link then arrives already showing that exact figure, with no flash of the default one. The directory, About and Privacy pages have no settings and are prerendered.
