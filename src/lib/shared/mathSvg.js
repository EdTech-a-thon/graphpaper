...
const UPPER = { A: 0.722, B: 0.667, C: 0.667, D: 0.722, E: 0.611, F: 0.556, G: 0.722, H: 0.722, I: 0.333, J: 0.389, K: 0.722, L: 0.611, M: 0.889, N: 0.722, O: 0.722, P: 0.556, Q: 0.722, R: 0.667, S: 0.556, T: 0.611, U: 0.722, V: 0.722, W: 0.944, X: 0.722, Y: 0.722, Z: 0.611 }
...
const isLetter = (c) => /^\p{L}$/u.test(c) && c !== 'π'
const width = (c, italic) => (italic ? ITALIC[c] : UPRIGHT[c] ?? UPPER[c]) ?? 0.55
 ...
 * @returns {{ w: number, asc: number, desc: number, items: object[] } | null}  null when there's nothing to write
 */
export function layoutMath(text, size, { suffix = '', upright = false } = {}) {
...
const empty = () => ({ w: 0, asc: 0, desc: 0, items: [] })

/** Put `part` (laid out from its own origin) at the end of `box`, raised by `rise`. */
function append(box, part, rise = 0, gap = 0) {
  const x = box.w + gap
...
function run(chars, size, italic, x, y) {
...
function strand(tokens, size, upright) {
  const box = empty()
  let pending = null // the run being built: { chars, italic }
  let prev = null // what came before, for spacing operators: 'op' | 'open' | 'item' | null
...
function fraction(top, bottom, size, upright) {
  ...
  const box = { w, asc: -(numBase - num.asc), desc: denBase + den.desc, items: [] }
...
function root(inner, size) {
  ...
  const box = { w: end + 0.02 * size, asc: -top + 0.04 * size, desc: bottom, items: [] }
  box.items.push({
    kind: 'path',
